...
class Model(nn.Module):
    ...
    def get_loss_iter(self, data, noises=None):
        ...
        if noises is not None:
            noises[t!=-1] = torch.randn((t!=-1).sum(), *noises.shape[1:]).to(noises)
...
def train(gpu, opt, output_dir, noises_init):
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        for i, data in enumerate(dataloader):
            ...
            noisy_pcs = noise_scheduler.add_noise(pcs, noise, timesteps)

            points = PointTensor(
                coords=noisy_pcs[:, :, :3],
                features=noisy_pcs.permute(0, 2, 1),
                is_channel_last=False,
                timesteps=timesteps,
            )

            # Predict the noise residual
            model_output = model.model(points).permute(0, 2, 1)
        ...
        if (epoch + 1) % opt.vizIter == 0 and should_diag:
            ...
            with torch.no_grad():
                ...
                x_gen_eval = model.gen_samples((25, 3, 2048), pcs.device, clip_denoised=False)
                x_gen_list = model.gen_sample_traj((1, 3, 2048), pcs.device, freq=40, clip_denoised=False)
            ...
            print("x_gen_eval", x_gen_eval.shape)
            visualize_pointcloud_batch('%s/epoch_%03d_samples_eval.png' % (outf_syn, epoch),
                                       x_gen_eval.transpose(1, 2), None, None,
                                       None)

            print("x_gen_all", x_gen_all.shape)
            ...
            print("--pcs", pcs.shape)
            visualize_pointcloud_batch('%s/epoch_%03d_x.png' % (outf_syn, epoch), pcs, None,
                                       None,
                                       None)
...
def parse_args():
    ...
    '''eval'''
    parser.add_argument('--saveIter', default=10, help='unit: epoch')
